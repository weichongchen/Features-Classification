...
%% 1.1
% 1.1.1
trainingDir = './Assignment06_data_expanded/TrainingDataset/';
...
testingDir = './Assignment06_data_expanded/TestDataset/TestDataset_';
classCount = zeros(length(histgram));
for n=1:length(histgram)
    imgage = dir(strcat(testingDir,num2str(n)));
    imgage = imgage(3:end);
    for i=1:length(imgage)        
        Img = imread(strcat(testingDir,num2str(n),'/',imgage(i).name));
        if(length(size(Img))==3)
            I = rgb2gray(Img);
        else
            I = Img;
